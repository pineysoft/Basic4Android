...
Sub LWM_OffsetChanged (Engine As LWEngine)
	If Engine.ScreenWidth > Engine.ScreenHeight Then
		image = LoadBitmap(File.DirAssets, "drawboardlandscape_complete.png")
	Else
...
Sub lwm_Touch (Engine As LWEngine, Action As Int, tx As Float, ty As Float)
	...
	Engine.Rect.Top = y - 10
	Engine.Rect.Left = x - 10
	Engine.Rect.Bottom = y + boxsize
	Engine.Rect.Right = x + boxsize
		
	Engine.Canvas.DrawCircle(tx, ty, 10dip, Colors.Black, True, 2dip)
...
Sub lwm_Tick (Engine As LWEngine)
	...
	If y > 0 Then
		...
		Engine.Canvas.DrawCircle(x,y,10dip, Colors.Black,True, 2dip) 'Draw the new smiley
		Engine.Refresh(Engine.Rect)		
	Else
		lwm.StopTicking
	End If
